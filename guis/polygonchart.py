...
class PolygonBacktestingChart(Chart):
    """
    ...
    def __init__(
        ...
        super().__init__(width, height, x, y, title, screen, on_top, maximize, debug, toolbox, inner_width, inner_height)
        ...
        table = self.create_table(width=0.3, height=1,
                  headings=('Symbol', 'date', 'stop', 'targets', 'trades'),
                  widths=(0.2, 0.1, 0.2, 0.2, 0.3),
                  alignments=('center', 'center', 'right', 'right', 'right'),
                  position='left', func=self._on_row_click)
    
        file_path = "/Users/vsai23/Workspace/PolygonBacktest/results/ep/targets_2023-01-01_2024-01-01.csv"
        if file_path:
            # get the strategy name from the file path
            self.strategy = file_path.split('/')[-2]
    ...
    def _on_row_click(self, row):
        ...
        trades = [(datetime.strptime(trade[0], "Timestamp('%Y-%m-%d %H:%M:%S')") + timedelta(hours=5), float(trade[1]), float(trade[2])) for trade in trades]
        
        self.end_date = (datetime.strptime(date.split(' ')[0], '%Y-%m-%d') + timedelta(days=self.strategy_times[self.strategy])).strftime('%Y-%m-%d')
        self.run_script(f'''
            {self.id}.search.window.style.display = "flex"
            {self.id}.search.box.focus()
            {self.id}.search.box.value = "{symbol}"
        ''')
        ...
        for trade in trades:
            ...
            self.marker(trade[0], 
                         ...
                         'red' if trade[2] < 0 else 'green')
            
        # draw horizontal line for targets
        for target in targets:
            self.horizontal_line(target, 'green')
            
        # draw stop
        self.horizontal_line(row['stop'], 'red')
        
        # draw vertical line for entry
        self.horizontal_line(trades[0][1], 'white')
